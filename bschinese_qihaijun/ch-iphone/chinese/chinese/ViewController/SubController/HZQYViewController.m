...
//    http://www.yuwen100.cn/yuwen100/hzzy/ios/hanziqiyuan/jsjs/index.html
    ...
    for (int i = 0; i <3; i++) {
        UIButton *button = [[UIButton alloc] initWithFrame:CGRectMake(30, 20+100*WithMode*i, 63*WithMode, 66*WithMode)];
        button.tag = i;
...
-(UIView *)creatHZYB{
    ...
    UIImageView *viewBk = [[UIImageView alloc] initWithFrame:CGRectMake(0, 20, 120*WithMode, 400*WithMode)];
    viewBk.image = [UIImage imageNamed:@"hzyb_menubg"];
    viewBk.userInteractionEnabled = YES;
    for (int i = 0; i <5; i++) {
        UIButton *button = [[UIButton alloc] initWithFrame:CGRectMake(20*WithMode, (35+80*i)*WithMode, 51*WithMode, 32*WithMode)];
        button.tag = i;
        [button addTarget:self action:@selector(showUrl:) forControlEvents:UIControlEventTouchUpInside];
        [viewBk addSubview:button];
        CGRect rect = button.frame;
        if (i==0) {
            rect.size.width = 70*WithMode;
            button.frame = rect;
        }
        if (i ==1) {
            rect.origin.y -= 5*WithMode;
            button.frame = rect;
        }
        if (i == 2) {
            rect.origin.y -= 5*WithMode;
            rect.origin.x += 5*WithMode;
            button.frame = rect;
        }
        if (i == 3) {
            rect.origin.x -= 10*WithMode;
