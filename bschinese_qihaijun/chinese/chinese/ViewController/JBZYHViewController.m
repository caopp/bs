...
-(void)showObjectWithId:(NSString *)mId{
    ...
    webVC.strUrl = [NSString stringWithFormat:@"http://www.yuwen100.cn/yuwen100/hzzy/ios/jbzy/%@/index.html",mId];
...
-(void)addnumb{
    ...
    float w = (self.view.frame.size.width*2/7-40)/5;
    for (int i = 0; i <11; i++) {
        
        UIButton *numb_btn = [[UIButton alloc] initWithFrame:CGRectMake(10 + w*(i%5), 40 + (w+10)*(i/5),w, w*4/5)];
        numb_btn.tag =  i+2;
